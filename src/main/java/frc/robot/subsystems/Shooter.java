...
public class Shooter extends SubsystemBase {
  ...
  private static final double kV = 0.54;
  ...
  private static final double ENCODER_DEVIATION = 1 / 42; // 1 tick of built in neo encoder
  ...
  private double getWheelSpeedL() {
    return driveL.getVelocity().getValue(); // Returns velocity in RPS.
  }

  @Log.NT
  private double getWheelSpeedR() {
    return driveR.getVelocity().getValue(); // Returns velocity in RPS.
  }
  ...
  public void periodic() {
    sysL.update(getWheelSpeedL()); // Returns RPS
