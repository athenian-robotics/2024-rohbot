...
public class PowerBudgetSim extends SubsystemBase implements PowerBudgetIO {
  ...
  private double getAdjustedCurrentDrawAmps(double currentDrawAmps, int channel) {
    ...
    if (channel >= 1
        && channel <= inputs.channelCount
        && channel <= inputs.pdpChannelCurrents.length) {
  ...
  public void report(final double currentDrawAmps, final int... channels) {
    ...
    for (final int channel : channels) {
      ...
      final double adjustedCurrentDrawAmps =
          getAdjustedCurrentDrawAmps(averageCurrentDrawAmps, channel);
  ...
  public double getTotalCurrentDraw() {
    return currentDraws.values().stream().mapToDouble(Double::doubleValue).sum();
  }
  ...
  public void periodic() {
    ...
    Logger.recordOutput(
        logKey + "/CurrentDraws",
        currentDraws.values().stream().mapToDouble(Double::doubleValue).toArray());

    RoboRioSim.setVInVoltage(
        Math.max(BatterySim.calculateLoadedBatteryVoltage(12, 0.02, getTotalCurrentDraw()), 0));
  }
